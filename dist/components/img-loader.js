...
var ImgLoader = (function () {
    function ImgLoader(_element, _renderer, _imageLoader, _config, _file) {
    ...
    ImgLoader.prototype.setImage = function (imageUrl, stopLoading) {
        ...
        if (this._useImg) {
            ...
            // set it's src
            this._file.resolveLocalFilesystemUrl(imageUrl).then(function (entry) {
                _this._renderer.setElementAttribute(_this.element, 'src', entry.toInternalURL());
            });
            ...
            this._file.resolveLocalFilesystemUrl(imageUrl).then(function (entry) {
                _this._renderer.setElementStyle(_this.element, 'background-image', 'url(\'' + (entry.toInternalURL() || _this.fallbackUrl) + '\')');
            });
        }
        this.load.emit(this);
    };
